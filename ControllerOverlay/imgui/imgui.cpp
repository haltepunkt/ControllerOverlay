...
void ImGui::PushMultiItemsWidths(int components_, float w_full)
{
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiStyle& style = GImGui->Style;
    if (w_full <= 0.0f)
        w_full = CalcItemWidth();
    const float w_item_one  = ImMax(1.0f, (float)(int)((w_full - (style.ItemInnerSpacing.x) * (components_-1)) / (float)components_));
    const float w_item_last = ImMax(1.0f, (float)(int)(w_full - (w_item_one + style.ItemInnerSpacing.x) * (components_-1)));
    window->DC.ItemWidthStack.push_back(w_item_last);
    for (int i = 0; i < components_-1; i++)
...
bool ImGui::SliderFloatN(const char* label, float* v, int components_, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    BeginGroup();
    PushID(label);
    PushMultiItemsWidths(components_);
...
bool ImGui::SliderIntN(const char* label, int* v, int components_, int v_min, int v_max, const char* display_format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragFloatN(const char* label, float* v, int components_, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragIntN(const char* label, int* v, int components_, float v_speed, int v_min, int v_max, const char* display_format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputFloatN(const char* label, float* v, int components_, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputIntN(const char* label, int* v, int components_, ImGuiInputTextFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const int components_ = alpha ? 4 : 3;
    ...
    if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one  = ImMax(1.0f, (float)(int)((w_items_all - (style.ItemInnerSpacing.x) * (components_-1)) / (float)components_));
        const float w_item_last = ImMax(1.0f, (float)(int)(w_items_all - (w_item_one + style.ItemInnerSpacing.x) * (components_-1)));
    ...
    if (window->DC.LastItemRectHoveredRect && BeginDragDropTarget()) // NB: The LastItemRectHoveredRect test is merely an optional micro-optimization
    {
        ...
        {
            memcpy((float*)col, payload->Data, sizeof(float) * components_);
            value_changed = true;
        }
        EndDragDropTarget();
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Setup
    int components_ = (flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4;
    ...
    memcpy(backup_initial_col, col, components_ * sizeof(float));
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        if (ref_col != NULL)
        {
            ...
            {
                memcpy(col, ref_col, components_ * sizeof(float));
                value_changed = true;
            }
        }
        EndGroup();
    ...
    return value_changed && memcmp(backup_initial_col, col, components_ * sizeof(float));
}
